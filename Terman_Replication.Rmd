...
fig2m <- sean_graph(list(nb1, wnb1), labels = c("All Data", "No Warzones"), title = "Marginal effects plot", xlabel = "Muslim Majority", ylabel = "Marginal Effect of Women's Rights on Coverage")
```
...
altered_text_data <- meta %>% 
  dplyr::select(COUNTRY_FINAL, YEAR) %>% 
...
fig3m <- sean_graph(list(nb1, wnb1, text_nb1), labels = c("All Data", "No Warzones"), title = "Marginal effects plot", xlabel = "Muslim Majority", ylabel = "Marginal Effect of Women's Rights on Coverage")
...
# GET ALL ARTICLE STHAT DO NOT INCLUDE A LISTED TERM IN SUBJET
meta %>% 
  dplyr::select(SUBJECT) %>% 
  mutate(subject = as.character(SUBJECT)) %>% 
  filter(!grepl(c("terroris", "bomb"), tolower(subject)))

meta %>% 
  dplyr::select(TEXT) %>% 
  mutate(text = as.character(TEXT)) %>% 
  filter(grepl(c("terroris", "bomb"), tolower(text)))

# We expect frequency of women related terrorist events to be associated with repressed womens rights, possibly biasing these results
