...
#### Method 1: War Hypothesis

Western media is more likely to report on countries that the US is engaged with in military conflict, which also happen to be muslim majority countries with poor women's rights records. I remove these countries from the dataset to account for this.

I created a new dataset that removed countries at war from the U.S. from Terman's initial dataset (NAW countries - Not At War). I then repeated Terman's initial models on the dataset. We first can see the probit and negative binomial models (Tables 1 and 2).
...
We can see that in both of these models, the expected positive correlation between Muslim Country and article count, and the expected negative correlation between Muslim Country x Women's Rights exist and are statistically significant. We can judge the significance of any changes in magnitude by looking at the marginal effects of women's rights coverage using a binary interaction plot (Figure 2).

## Figure 2: Marginal Effects of Women's Rights Index on Reported (Count). All Countries vs. NAW Countries
...
From the marginal effects plot we can see how by removing NAW countries, the magnitude of the effects described by Terman was reduced. More importantly, the confidence intervals of the Non-Muslim Countries now passes below 0, making the conclusion that Women's Rights increase the probability of receiving an article less significant.

## Figure 3: Marginal Effects of Women's Rights Index on Reported (Count). All Countries vs. NAW Countries
...
TBD whether this is possible or not: might require some heavy lifting
