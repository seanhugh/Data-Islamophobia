...
- **NAW Countries**: I create a new dataset which discards articles regarding countries that the US is currently in active military conflict with. I label these countries NAW (Not At War) countries. The countries that are excluded are Afghanistan, Iraq, Syria, Yemen, Somalia, Libya, and Niger (@vice). In the initial data set, there are 6448 country-year datapoints (where each datapoint represents a given country in a given year). In the new dataset there are 6209.

- **Terrorism Related Terms**: I remove articles whos subject contain any of the following terms related to terrorism: terroris", "bomb", "attack". This list was created by reading through articles, selecting ones which covered terrorist content, and examining their shared subject tags. In the initial dataset there are 4531 articles. After removing articles related to terrorism there are 4253 articles.

#### Modeling Methods

Terman uses 2 primary methods of modeling the data: probit model and negative binomial model:\newline

- **Probit Model**: A probit regression is used because we are dealing with a binary variable (has an article written or does not have an article written).
...
Articles like the one in figure 5 match Terman's criteria of being articles that fall under the category of women. However, it can be assumed that terrorist acts would generally be reported on regardless of location. Given that all of the top 6 countries most affected by terrorism are Muslim majority countries [@forbes], and that terrorism can be seen to impat womens' rights, it is reasonable to assume that terrorism could impact Terman's results. I remove articles related to terrorism from the dataset and analyze the data using Terman's methods of analysis. We can see the results in figure 5.

## Figure 5: Marginal Effects of Women's Rights Index on Reported (Count). All Articles vs. Excluding Terrorism
...
While there were some minor reductions in the magnitude of the impact of womens' rights on article countr in muslim countries, they were negligible compared to the results of removing AW countries. We can further analyze this data by creating a new dataset of NAW countries with terrorism removed (essentially combining hypothesis 1 and 2). We will call this new dataset "NAW Terrorism Removed". The results when modeling this dataset are below in figure 6.

## Figure 6: Marginal Effects of Women's Rights Index on Reported (Count). All Data vs. NAW / Terrorism Removed Data
...
Were I further investigate Terman's work I would be interested in finding indicators of war (in regard to the US) and terrorism that could be properly used to act as indicator variables in the models. For war, one possibility could be "# of deployed US soldiers in that specific country" (Assuming that it is a country with active military engagement not a base in a foreign ally's country).

Another interesting trend I found was the increase in articles written about women in countries at war with the US after those wars began. It would be interesting to further examine the data above on a before and after timeline to see how the results change when examining the data before and after the conflict initiated.
